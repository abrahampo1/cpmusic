...
<body id="back" onload="color()" style="text-align: center;">
    ...
    <div style="display: flex; width:100%; background-color: none">
        ...
        if ($contenido == false) {
            ...
            echo '<div class="video-div"><input type="hidden" id="video_tiempo" value="' . $tiempo . '"><input type="hidden" id="video_total" value="' . $tiempo_total . '"><video src="' . $videourl . '#t=' . $tiempo . '" id="videoclip" onload="barra()" onloadstart="barra()" autoplay width="100%" height="auto"></video><div id="myProgress">
            <div id="myBar"></div>
          </div></div>';
        }
...
<script>
    var anuncios = window.setInterval(function() {
        $.ajax({
            ...
            success: function(response) {
                if (response != "" && strlen(document.getElementById("noticia").innerHTML)<10){
